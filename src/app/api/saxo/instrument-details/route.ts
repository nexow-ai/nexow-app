...
export async function GET(request: NextRequest) {
  ...
  const url = `${NEXOW_API_URL}/saxo/instruments/details/${uic}/${encodeURIComponent(assetType)}`;
  try {
    const res = await fetch(url, { cache: "no-store" });
    const data = await res.json();
    if (!res.ok) {
      return NextResponse.json(
        { error: data.detail ?? "Failed to load details" },
        { status: res.status }
      );
    }
    return NextResponse.json(data);
  } catch (e) {
    console.error("Saxo instrument details error:", e);
    return NextResponse.json(
