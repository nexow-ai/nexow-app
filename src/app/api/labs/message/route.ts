...
export async function POST(request: NextRequest) {
  ...
  const creditCost = LAB_MODEL_CREDITS[model as string] ?? LAB_DEFAULT_CREDITS;

  // Consume credits (checks balance atomically and deducts)
  const { data: consumed, error: creditError } = await (
  ...
  try {
    const engineResponse = await fetch(`${NEXOW_API_URL}/api/labs/message`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        messages,
