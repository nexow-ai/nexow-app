...
import { ArrowUp, ChevronDown, FlaskConical, Loader2 } from "lucide-react";
        ...
        <div className="flex h-full flex-col">
            ...
            <div className="flex items-center justify-between border-b border-zinc-800/40 px-5 py-3.5">
                ...
                <div className="relative" ref={pickerRef}>
                    ...
                    {showModelPicker && (
                        <div className="absolute right-0 top-full z-50 mt-1.5 w-72 rounded-xl border border-zinc-800/60 bg-zinc-950 p-1.5 shadow-2xl animate-fade-in">
                            {LLM_PROVIDERS.map((p) => (
                                <div key={p.id}>
                                    ...
                                    {p.models.map((m) => (
                                        <button
                                            ...
                                            className={`flex w-full items-center justify-between rounded-lg px-2.5 py-2 text-left text-xs transition-colors ${provider === p.id && model === m.id
                                                ? "bg-purple-500/10 text-purple-400"
                                                : "text-zinc-400 hover:bg-zinc-800/50 hover:text-zinc-200"
                                                }`}
                                        >
                                            <span className="flex items-center gap-2">
                                                <span className={`h-1.5 w-1.5 rounded-full ${provider === p.id && model === m.id
                                                    ? "bg-purple-400"
                                                    : "bg-zinc-700"
                                                    }`} />
                                                {m.name}
                                            </span>
                                            <span className={`rounded-md px-1.5 py-0.5 text-[10px] font-medium tabular-nums ${provider === p.id && model === m.id
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
                <div className="space-y-4">
                    ...
                    {/* Error */}
                    {error && (
                        <div className="rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-3 text-xs text-red-400 animate-fade-in">
                            {error}
                        </div>
                    )}
            ...
            <div className="border-t border-zinc-800/40 p-4">
                <form onSubmit={handleSubmit} className="relative">
                    <textarea
                        ...
                        className="w-full resize-none rounded-xl border border-zinc-800/60 bg-zinc-900/50 py-3 pl-4 pr-12 text-sm text-zinc-100 placeholder:text-zinc-600 transition-all focus:border-purple-500/50 focus:outline-none focus:ring-2 focus:ring-purple-500/20 disabled:opacity-50"
                    />
                    <button
                        type="submit"
